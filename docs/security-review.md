...
- **非破壊の不変条件**:
  ...
  - **削除 API は存在を確認したが公開しない**(テスト後始末でのみ直接使用)
...
- **`add_pine_to_chart` は追加のみ**: スタディの削除・置換 API は公開しない(誤指定でユーザーのチャート構成を壊すリスクの方が大きい)。追加されたスタディはユーザーが UI からいつでも外せる
...
- 引き続き、既存スタディの削除・置換・チャートからの除去はこのツールの範囲外(`add_pine_to_chart`同様、ユーザーのチャート構成に対する書き込みは追加専用の原則を維持)
