...
### #100 HMM 研究系列(研究実施, 2026-09-12〜09-23)
...
- **結論**: 日中足では、HMM状態を直接の売買方向にしても既存シグナルのゲートにしても、2bps後に一貫してPF 1を超える構成はなかった。日足では上昇相場でPF 1を超えるものがあったが、同じ約定での always-long を上回ったのは FX 3/25、先物 3/30 にとどまる。長期校正では、12か月の履歴で800件の窓が埋まり、エントリー信頼度ヘッドの局所支持が64.66%から81.97%に上がった(支持のみ、損益は未測定)
- **残**: (1) 800窓の支持が損益にどう効くかを測る性能実験(別途の設計・レビュー・承認と結果追跡の区切りが必要)、(2) scheduled-closure 規則の実モデル統合、(3) hmm-d1-price-v1 の weekend 判定を時刻述語に置き換える v2 は PF に影響しないため保留
