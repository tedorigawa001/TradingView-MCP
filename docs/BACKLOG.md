...
- **外部実装の評価 — 採用しない(2026-09-22、HKUDS/AI-Trader `research/`)**: FDR・ブートストラップCI・DiD・HTEを標準装備すると謳う研究パイプラインを読んだが、統計的中核が動作していない。`bootstrap_ci` は復元抽出ではなく添字の回転で、`gcd(7,n)=1` では全反復が元の平均に厳密一致するため **CI 幅が 0**(n=5/10/20/50/100 で実測確認)。`CONTROL_COLUMNS` の先頭が `max_drawdown` で同じ列が指標にもなるため回帰が自分自身を説明変数にして β=1、調整後は全行が総平均。DiD は `p_value=1.0` 固定のまま BH 族へ投入される。BH の step-up 実装自体は正しいが、同一データに4手法を回して1族にまとめるため名目 m が冗長で膨らむ — **#46 が測ろうとしている問題の実例であって解決策ではない**。副産物として一点確認できたのは、候補ゼロが続く現状で BH へ替えても判定は動かないこと: 全帰無下で BH は FWER を α で制御し、判定を決める最小p値の閾値は rank k=1 で `α/m` と Bonferroni に一致する(BY はさらに `H(m)`≈4.5倍厳しい)。FDR は真の発見が多数ある場面の道具であり、**FWER を選んでいる現状の設計が正しい**。
...
### #87 リポジトリ全体の脆弱性チェック(結論: 3件修正+依存更新, 2026-09-05)

- **範囲**: 差分ではなくツリー全体。依存関係、CDP トランスポート、`Runtime.evaluate` の全27呼び出し、外向き fetch の全13経路、ファイル境界、秘密情報の取り扱い
- **A. デバッガソケットの宛先が未検証だった(Medium, 修正済み)**: security-review #3 は**ページ URL** の検証を固めたが、その直後に開くソケットは別のアドレスであり、誰も見ていなかった。`/json` が本物の `https://www.tradingview.com/chart/...` を返しつつ `webSocketDebuggerUrl` を任意のホストへ向けると、`connect()` はそこへ繋いだ。**PoC で実証済み** — クライアントは攻撃者が指定したリスナーへ接続した。Pine ソースやアラートのペイロードを含む送信内容がマシン外へ出るため、#1 で受容した「ローカルプロセスは既にセッションを掌握している」モデルには収まらない。むしろ運用ルール「使う時だけデバッグポートを開く」を突く — **ポートが閉じている時間帯こそ、他プロセスが先に掴める**。`assertDebuggerWebSocketUrl` で接続前に `ws:`/`wss:` と host:port 一致を要求(loopback 3表記のみ相互許容)
- **B. パースできない CDP フレームでプロセスが死んだ(Medium, 修正済み)**: `ws` の message ハンドラは同期なので、内部の `JSON.parse` の throw は未処理リスナー例外になり、`uncaughtException` ハンドラはリポジトリ内に一つも無い。ピアからの不正フレーム1つでサーバーが終了した。`src/` の `JSON.parse` 30箇所のうち、try でも await 経路でもなかったのはここだけである
- **C. `redactSecrets` が二次オーダーだった(Medium, 修正済み)**: `://` 前の `[\w+.-]*` が、スキームでなかった文字ごとに残り全体を走査していた。**実測 40 KB=2.9秒、80 KB=11.7秒、200 KB=72秒**。この関数は**全ツールエラー**(`errorResult`)と全ページ例外を通るため、長いメッセージ一つで Node の単一スレッドが凍る。ページが自分の例外文字列を選べる時点で成立し、A があれば不正ピアから直接届く。`{0,64}` で走査を打ち切り(**200 KB が 112ms**)、出力も 4096 字で切る。**redact してから切る**順序 — 先に切ると秘密が切断点をまたいだとき接頭辞が残る
- **D. 依存(修正済み)**: `fast-uri` 3.1.5(high, SSRF・host confusion)と `qs` 6.15.3(moderate)。`npm audit fix` で 3.1.7 / 6.16.0 へ。`package.json` は無変更
- **変異検証**: 5つの欠陥を `build/` へ戻し、全てテストが落ちることを確認した(`npm test` は `pretest` で再ビルドして変異を消すため `node --test` を直接使用)。C の時間テストは**修正後112ms / 欠陥時71,981ms** と650倍の余裕がある
- **既存レビューの穴として学んだこと**: #3 は「ページ URL の検証」で止まり、その先の接続を見ていなかった。#5 はページ由来文字列をプロンプトインジェクションの面だけで Low と評価し、**同じ入力の DoS の面**を見ていなかった。#4 の「`npm audit`: zero」は表に残ったまま陳腐化し、今回まで誰も突き合わせていない
...
- 最初は学習期間だけを1/3/5年で比較する。M15・既存6銘柄、共通評価期間2024〜2026年上期、年次再学習、既存特徴量・4時間保有・次足始値・コスト2bpsを維持する。学習ラベルの決済時刻は評価開始より厳密に前とし、学習用標本は4時間間隔。バー本数を独立標本数と呼ばない。
- 方向予測とメタラベリングを別々に測る。学習内/評価側のBrier score、定数確率対照、件数、コスト後PF・期待値・年別結果を併記する。既に閲覧した期間なので探索であり、独立OOSや採用根拠とは呼ばない。
- 次段階は時系列の内側検証による限定探索。学習率・葉数・最小葉標本数・正則化・反復数の探索予算を結果を見る前に固定し、外側の同一期間へ適用する。内側の選択を含む監査は未実装。固定台帳の選択監査は再学習を監査していない。
- 研究コード・設定・測定結果はGit対象外の `playbook/ai-top5/` に保存し、公開リポジトリには汎用基盤と本課題のみを残す。凍結済み金M15前向き契約・モデル・期間は変更しない。
- 初段の学習曲線ランナーを実装。サブエージェントの独立レビューでBrier評価母集団の説明不足と終了時のみのコードハッシュを指摘され、4時間アンカー限定の明記・開始時ハッシュと終了時一致検査へ修正した。36テスト通過。期間比較の結果・台帳は非公開研究領域に保存し、限定パラメータ探索とDC変種は未着手。
- 限定パラメータ探索へ着手: 元の3年学習を維持し、方向ML6設定・メタ正則化3設定を、各評価年の直前1年の半年2foldで選ぶ(各内側fitも過去3年)。選択基準は同一4時間アンカーの件数加重Brier、同点は宣言順。外側損益による選択・閾値探索・設定失敗の黙示除外は禁止。既に閲覧済みの履歴なので独立OOSとは呼ばず、固定設定と一次モデル単独も別途評価する。詳細契約・コードは非公開研究領域に保存する。
- 限定探索の実装・測定・照合完了 (2026-09-07): 内側324fit、36選択、外側90レコード。サブエージェントレビューで入力束縛・境界テスト・独立照合の不足を測定前に修正。45テスト、全43,295取引の元価格・費用・標本照合、固定設定15,327取引の前回再現を確認した。結果は非公開 `NESTED_PARAMETERS_REPORT.md` に記録。選択手続きを含むヌル校正と未使用期間は未実施であり、候補ゲートは閉じたまま。凍結済み前向き成果物は不変。

### #89 メタラベリング改善 (構想, 2026-09-07)

- #88の学習期間比較を先行する。その後、一次モデル単独との同一機会集合対照、採用率・除外取引の損益、確率較正、LogisticRegressionの正則化を限定比較する。閾値だけの最適化とモデルの改善を区別する。
- ラベルは一次方向のコスト控除後利益の有無。一次モデル自体を学習型に変更する場合は、メタ学習用の一次予測を時系列out-of-foldで生成し、学習内予測を使わない。特徴量・ラベル・出口変更は別契約とする。
- 内側検証で選択し、探索全体の多重度・選択バイアスと未使用期間を検証するまで候補化しない。凍結済み金M15追試へ合流させない。
- 初段の学習期間比較と正則化3設定の内側選択、一次モデル単独の対照実行は#88で測定済み。占有時間が変わるため対照との差を同一取引の因果効果とは呼ばない。同一機会集合の採用/除外比較、確率較正・追加特徴量は未実装。
- 同一機会集合の比較へ着手 (2026-09-07): 一次モデルの4時間占有を採用/除外/欠測に関係なく固定し、元の3年学習・C=.1・閾値.55だけを適用する。後続シグナルで穴埋めしない別政策として、採用/除外の損益、全完了機会あたり損益、確率の記述的較正を照合する。前回の一次モデル台帳の完全再現を要求し、欠測をゼロ利益へ置換しない。既存のフィルター戦略とは占有が異なるため改善の因果推定とは呼ばず、詳細契約と結果は非公開研究領域に保存する。
- 同一機会比較の実装・測定・独立照合完了 (2026-09-07): 23,266試行機会を照合し、完了17,923取引は一次モデルの前回台帳と完全一致。全体/採用/除外の費用別損益合計、欠測、同一機会母集団のBrier・固定確率binを記録した。独立レビューの証跡・因果性テストの不足を測定前に補正し、63テスト通過。詳細結果はGit対象外の `META_OPPORTUNITY_REPORT.md` へ保存。採用側は少数で、欠測の影響も残るため候補化しない。追加特徴量、確率の再較正、学習選択を含むヌル監査、未使用期間での再現は別の残課題。凍結済み前向き契約は不変。

- 同一機会パラメータ探索へ着手 (2026-09-07): 正則化3値と閾値3値の9設定に限定し、各評価年の直前1年・半年2foldで選択する。各内側fold30件以上の採用と全完了機会あたりnet2bps損益が正であることを要求し、満たさなければ年単位で見送る。内側108fit、外側最大36fit。閾値の引下げを含む新しい探索であり、前回の固定契約を上書きせず未使用OOSとも呼ばない。実装/独立レビュー/テスト後に測定し、詳細はGit対象外。
- 限定チューニングの測定・照合完了 (2026-09-07): 70テスト、内側83,931行/外側46,532行の照合、18選択の別計算、前回固定23,266機会の再現を確認。実行環境版と件数不足/非正損益による見送り理由をレビュー後に追記してから測定した。結果は非公開 `META_TUNING_REPORT.md` へ保存。今回の限定探索から明確な選別改善は認めず、候補ゲートは閉じたまま。期間・閾値を結果後に拡張せず、既存前向き追試は変更しない。

### #90 Directional Change改善 (構想, 2026-09-07)

- DCは現状では学習モデルでなく固定反転閾値のルールであり、学習反復を増やしても変わらない。固定bps対照と、過去データのみで推定してイベント開始時に固定するボラ正規化閾値を別契約で比較する。
- イベント時計のduration/overshoot特徴量はシグナル時点までに確定した値だけを使う。完成後の極値を過去シグナルへ遡及付与しない。ギャップで状態リセット、確認後の次足約定、重複保有排除を維持する。
- 閾値尺度の比較を先行し、保有期間・出口・方向モデルの同時探索を避ける。対照、時間分割、費用、最低件数、探索予算を凍結してから実装・測定する。現時点は未測定。
- 閾値比較v1へ着手 (2026-09-07): 既存20bps再現、共通品質条件の固定20bps/10bps、過去ボラに基づく区間開始時固定閾値の4政策を、既存M15・6銘柄・2024〜2026年上期で比較する。品質/リセット変更と閾値効果を分け、係数や出口は掃引しない。詳細契約・実装は非公開研究領域。テストと独立レビュー後に測定し、前向き契約は変更しない。
- 閾値比較v1の実装・測定・独立照合完了 (2026-09-07): 原本実バイト再検証と必須証跡ハッシュ集合の指摘を測定前に修正し、56テスト通過。別のモーメント計算・first-hit状態再構成で65,206イベントと28,798取引を照合、旧20bpsの6,243取引は前回と完全一致。72年別レコード・24銘柄政策集計を確認した。結果は非公開 `DC_THRESHOLD_REPORT.md` に保存し、候補化しない。係数/出口の追加掃引は行わず、前向き契約は不変。DCのduration/overshoot特徴量の学習利用は未実装で、本比較の完了とは区別する。

### #91 保存済み取引台帳の絞り込み・再集計 (着手, 2026-09-07)

- 登録済み成果物IDから銘柄の包含/除外、売買方向、期間、銘柄/年/月別の再集計を行う。PFは銘柄別PFの平均でなく取引の利益合計/損失合計から再計算する。件数、欠測、費用、単位、時間基準、出自ハッシュを返す。
- 初版は明示的なローカルCLIインポートによる不変台帳と読み取り専用MCP。入力はgross return bps契約に限定し、通貨建てPnLや費用控除済み損益を暗黙変換しない。MCPはパス・コードを受け付けず、チャート・注文に触れない。期間/年/月は決済時刻UTCを基準とし、欠測をゼロ利益にしない。
- 上限、重複ID、非有限値、時刻不整合、ハッシュ不一致、symlinkを拒否する。元の研究コード/台帳は非公開のまま、公開対象は汎用機能のみ。単体・MCP統合テストとサブエージェントレビュー後に利用する。
- 初版実装・検証完了 (2026-09-07): `summarize_backtest_ledger`、不変ハッシュ台帳、確認必須インポートCLI、単位/欠測を保つ集計を追加した。9単体テストとMCP実ストア統合テストを追加し、最終全体944 Nodeテスト・Java各スイートが通過。サブエージェントが指摘したFIFO置換時の停止とnpm bin symlink経由の無実行を修正し、後者は修正前の失敗も確認した。非公開研究台帳で既存集計との一致をMCPインメモリ接続経由で確認済み。README/セキュリティレビュー/入力契約を更新。稼働中MCPへの反映には再起動が必要。cash PnLの変換や任意の研究形式の自動認識は初版に含めない。

- npm 0.1.5配布前検証 (2026-09-07): 全944 Nodeテスト・Java各スイート・raw-to-Javaテスト通過、実行時依存のnpm auditは0件。配布物145ファイルに研究データ/設定/ソースマップがないことを確認。一時環境へのtgzインストール後、bin経由のインポートとstdio MCP集計を検証し、サーバー版0.1.5・102ツール・合成台帳の期待値一致を確認した。デスクトップ側の再接続確認とは別の検証。

### #92 同一取引機会でのフィルター比較 (構想, 2026-09-07)

- primary opportunity IDで採用/除外/見送り/欠測を突合する。占有時間を固定した政策と再エントリー可能な政策を混同せず、全完了機会あたりの損益を併記する。

### #93 実験結果の差分説明 (構想, 2026-09-07)

- 成績変化と取引数・銘柄構成・期間・費用・占有時間の違いを表示する。構成差と同一取引の差を分離し、集計上の関連を因果効果と呼ばない。

### #94 限定チューニングの実行管理 (構想, 2026-09-07)

- 既存walk-forward/一括バックテストを拡張し、探索範囲、内側fold、選択基準、試行数、見送り理由、チェックポイントを固定保存する。失敗した設定を黙って除外せず、外側成績で選択しない。

### #95 検証証跡の一括チェック (構想, 2026-09-07)

- データ/コード/契約/実行環境版の一致と前回台帳の再現を確認する。内容の整合性と情報源の真正性を区別し、ハッシュ一致だけで統計的妥当性や採用可否を認定しない。

### #96 Sweep後の板回復と同一機会対照 (研究着手, 2026-09-07)

- 既存の6E Sweep検出器を共用し、確認後の同じ気配・占有時間で無条件/条件適合/除外を比較する開発研究に着手。条件確認前の価格への遡及エントリー、欠測depthのゼロ扱い、フィルターで空いた枠への置換エントリーを禁止する。
- 新手法の条件・コード・結果はGit対象外の `playbook/6e-refill-study/` に隔離する。公開対象は必要になった汎用MCP機能のみ。既存JAR・凍結済み研究・候補ゲートは変更しない。
- 巨大/追記中の生データは既存リプレイ上限を緩めず、上限付きの固定prefix証跡として明示する。部分標本・最新listener時計・遅延配信・連続性未証明を報告し、料金未設定なら純利益を主張しない。合成回帰テストと独立レビュー後に開発用coverageを確認する。

- #96実装・検証 (2026-09-07): 非公開の同一機会比較器と上限付きprefixランナーを追加。研究用13件・既存Bookmap関連14件・Java各スイートとraw-to-Javaテストが通過。独立レビューで条件欠測/決済欠測の分離、検出器バイナリの実行前後ハッシュ、古い気配拒否テストの独立性を修正した。実データ実行は外部ボリュームの読み取り待ちで中断し、最終レポート未生成。0件評価や収益性の結果とは扱わない。読み取り応答・停止プロセスを確認後に別runで再実行する。

- #96再開保護の検証 (2026-09-07): 非公開ランナーへ処理単位の期限、プロセス群停止確認、固定入力チェックポイント、原本/保存コピー/検出器のハッシュ照合を追加。研究用27件・既存Bookmap関連14件が通過し、独立再レビューで3指摘の解消を確認。実データ再実行は最初の原本読み取りが期限超過し、後続を未試行として保存した。終了待ちのプロセス群は後で消滅を確認済み。研究結果は未取得であり、条件変更やゼロ件評価とは扱わない。外部ボリュームの読み取り安定性の確認が残る。

- #96内蔵コピー経由の部分検証 (2026-09-07): 3原本を内蔵ディスクへコピーし、原本/コピーのSHA-256一致を確認して既存検出器を再実行した。残る1原本は全体コピー・限定prefix読み取りとも期限超過のため未評価。コピー台帳と研究結果を結合したcoverageへ未取得原本も明示し、4原本全体の評価完了とはしない。非公開の条件・結果は引き続きGit対象外。

- #96研究保留 (2026-09-07): ユーザー判断でCME・Sweep研究はいったん中断。既存の原本、内蔵コピー、未評価状態、研究証跡を保持し、収集ジョブ自体は変更しない。

### #97 CME非依存の東京仲値前モメンタム (研究着手, 2026-09-07)

- スポットFXの既存M1履歴を利用し、制度時刻に基づく固定条件と同一日付・同一保有時間の無条件対照を比較する。時刻変換、日本の休日、欠損窓、仮定費用、対応付き日次ブロック区間を明示する。
- 過去に探索済みの改訂履歴であり、未使用OOSや実約定の証拠とは扱わない。候補ゲートは閉じたまま、結果を見て条件を調整しない。
- 条件・コード・生データ参照・結果はGit対象外の `playbook/tokyo-fix-study/` に隔離。純関数・時刻/取込/集計テストと独立レビューの後で測定する。汎用機能が必要になった場合のみMCP側へ追加する。
- #97初版の実装・探索測定完了 (2026-09-07): 22テストが通過。少数標本ブートストラップの退化を修正し、独立再レビュー済み。原本ハッシュ・既存取込件数の一致と実行前後の不変性を確認し、保存台帳から平均/PFを独立再計算した。仮定費用下で採用根拠は得られず、候補ゲートは閉じたまま。無条件対照への損失削減と予測力を区別して非公開レポートに記録。結果に合わせた時刻/閾値の変更は行わない。

### #98 研究探索の来歴・対照・採用判断を支える機能 (構想, 2026-09-08)

- 背景: 不変台帳の内容ハッシュは改変検知であり、結果を見てから好成績のスライスを選んでいない証明ではない。#91の `ledger_records` と `selected_fraction` による分母表示を土台とし、以下の優先順で整備する。今回はバックログ記載のみで、候補契約や実行中の前向き研究は変更しない。

1. **スライス探索台帳 (最優先)**: 既存研究ジャーナルを拡張し、研究ID・元台帳ID・正規化した包含/除外銘柄、方向、期間、group_by、費用条件を記録する。総呼び出し回数と異なる条件数を分け、同一条件の再実行を新しい探索条件として数えない。group_byで一度に提示された比較対象も記録する。記録開始前・ツール外の探索は把握できないことを明示し、観測した試行数を探索全体の保証や自動的な多重度補正に使わない。正規化、再実行、グループ展開、記録失敗時の扱いを実装前に定義・テストする。
2. **同一台帳の対照比較 (高、#92・#93の拡張)**: 全体・選択群・除外群を一度に返す。件数、欠測、費用、PF、平均損益に加え、共通機会あたりの損益を区別し、選別改善と取引削減による損失/費用回避を混同しない。群の排他性、全体への件数・損益の再構成を検証する。PFを群別PFの平均で再構成しない。
3. **未使用期間の利用管理 (高)**: 研究ID、データ系列・版、参照期間、参照日時、探索/検証目的を記録し、探索済み期間のOOS再利用を検知する。台帳に記録が無いことだけで「未使用」を認定せず、過去の手動参照・別研究・ツール外利用の申告と不明状態を持つ。期間の部分重複をテストする。
4. **損益分岐コストの集計 (中、#91の拡張)**: PFとともに何bpsの往復費用まで平均純損益が非負かを表示する。初版は既存の一律往復費用契約に限定し、欠測除外、総費用の単位、負のgross平均、ゼロ損益を明示する。実スプレッド・スリッページや実約定可能性の保証とは扱わない。
5. **実行環境・契約の差分確認 (中、#95の拡張)**: 前回と今回のデータハッシュ、コード/runner版、候補規則、パラメータ、実行環境を比較し、変更点と再検証の必要性を表示する。入力不変性と統計的校正は別に判定し、情報不足なら互換性を断定しない。ハッシュ不変でも判定コードが変わるケースを回帰テストに含める。

- 共通方針: 汎用MCP機能のみを公開し、手法・パラメータ・研究結果は引き続きGit対象外。実装後は回帰テストと独立レビューを行い、探索来歴の可視化だけで `candidateEligible` を有効化しない。

- スライス探索台帳v1の実装契約 (2026-09-08): `summarize_backtest_ledger` の任意 `research_id` 指定をローカル記録への明示的オプトインとする。省略時は `exploration.status: untracked`、指定時は保存完了後だけ成績を返し、保存失敗を未追跡成功へ落とさない。IDは探索用名前空間であって仮説の事前登録ではない。既存の凍結研究ジャーナルは変更せず、共用first-seenストアのロック・耐久化を使う別ストリームへ保存する。
- 計数範囲は研究IDと台帳artifact IDの組。銘柄集合は順序/重複を正規化し、期間・方向・group_by・費用を条件ハッシュへ含める。呼出回数は再実行でも増え、異なる条件数は同一条件の再実行では増やさない。空の選択も記録し、成績非公開の不正入力は数えない。通信断直前に保存された呼び出しは受信未確認でも数え得る。グループキーと累計提示セル数は記録するが独立検定数とは扱わない。過去/外部/未追跡呼び出し・別IDの探索を把握できるとは主張しない。
- スライス探索台帳v1実装・検証完了 (2026-09-08): `backtestSliceJournal` とMCP接続を追加。条件ハッシュ、分母、選択件数/割合、提示グループを保存し、呼出回数・異なる条件数・記録開始日時を返す。32MiBファイル上限、破損/書きかけJSONL、時計逆行、所有権/シンボリックリンクを拒否。ビルドと台帳/サーバー207テスト通過後、ファイル上限の回帰1件を追加して保存層12テスト再通過。独立レビューで構文・ID制約の不一致を修正し、実ストア統合テストで空集合の検証不整合を修正、最終再レビューは追加不具合なし。README・保存契約・security review更新済み。ライブMCPの再接続後確認は未実施。優先項目2〜5は未着手。

- 同一台帳対照比較v1実装・検証完了 (2026-09-09、#92・#93): `summarize_backtest_ledger` に固定 `comparison` を追加。全フィルター前の台帳全体/選択群/厳密な除外補集合を同一費用で再集計する。日付フィルターも選択条件であってbaselineを狭めない。各群の平均と、全体の結果既知件数を共通分母にした政策損益を分離し、net差をgross差と回避費用へ分解。全体の欠測は比較をpartialにし、全欠測では平均をnull、既知機会をすべて見送った政策のみゼロとする。代替取引・再投資・因果効果・有意性・候補認定は主張しない。
- 比較表示の `comparison_contract` を探索台帳へ任意フィールドとして記録し、旧記録は表示済みと遡及認定しない。固定表示の追加は条件ハッシュを変えず、grouped-cellは既存group_by提示だけを数える。ビルド、台帳/探索記録/MCPサーバー212テスト、独立レビューと追加差分再レビューが完了。README・保存契約・security review更新。ライブMCPでの新比較応答は再接続後の確認待ち。優先項目3〜5は未着手。

- 期間利用管理v1へ着手 (2026-09-09): 明示的な `record_research_period_usage` と `check_research_period_usage` を追加する。記録はconfirm:true必須、実際の参照日時・研究ID・安定系列ID・版ハッシュ・UTC半開区間・探索/検証目的を保存する。同じaccess_idの同一定義再送は冪等、矛盾は拒否。別研究・別版でも同じ系列の部分重複を検出し、隣接のみは重複としない。検証目的の参照も既知の利用として扱う。
- 記録は利用者報告であり独立観測ではない。記録なし/未使用申告のみではunused_provenを有効化しない。シリーズ別名・派生系列・過去の外部参照は自動照合できず、lookback/将来結果を含む実際に見た範囲を報告する。初版は明示報告と確認のみで、既存研究ツールの自動計測・OOS実行拒否・期間予約を実装したとはしない。読み込み失敗を空台帳に落とさず、原本台帳や凍結仮説は変更しない。

- 期間利用管理v1実装・検証完了 (2026-09-09): 2つのMCPツール、ローカル追記台帳、部分重複/別研究/別版照合、冪等再送を追加。保存層16件を含む台帳比較・探索記録・MCP統合230テストとビルドが通過した。独立レビューで、保存時のfsync失敗後に再送が耐久化未確認のまま成功し得る点を修正。ファイル/ディレクトリ同期の障害注入テストを追加し、再レビューで解消を確認した。README・利用契約・security review更新済み。記録なしは未使用の証明ではなく、自動追跡・OOS実行拒否・予約は未実装。クラッシュ後の残留ロック回復は手動、Windowsではディレクトリfsyncは非対応。ライブMCPでの新ツール確認は再接続後に実施する。優先項目4・5は未着手。

- 期間利用管理v1ライブ確認 (2026-09-09): 実MCPから合成専用series/research IDで空台帳確認、新規報告、同一access_idの冪等再送、別版の重複検出、隣接区間の非重複を確認。再送後もsequence=1、unused_proven/candidateEligibleはfalse。合成報告1件はローカル台帳に保持し、実研究の利用履歴とは区別する。チャート・注文は操作していない。
- 損益分岐コストv1実装・検証完了 (2026-09-09): `summarize_backtest_ledger` の全体/選択/除外/各グループに `break_even_cost` を追加。結果既知取引のgross平均を非負の一律往復費用の上限とし、負のgross平均は上限null・negative_gross_mean、全欠測/空集合は算出不能、ゼロは有効な上限0とする。要求費用との差は符号付きheadroomとして表示。欠測をゼロにせず、標本平均を信頼限界や実約定可能性と呼ばない。既存条件ハッシュ・比較契約・候補ゲートは不変。新規テストの修正前失敗を確認し、レビュー指摘の注意事項リストと複数群アサーションを補正後、ビルド・関連231テスト・独立再レビューが完了。README/台帳契約更新済み。新集計のライブ反映はMCP再接続後。優先項目5の実行環境・契約差分確認は未着手。
...
### #100 HMM 研究系列(研究実施, 2026-09-12〜09-24、結論: 打ち切り)

- **範囲**: 8日間で `playbook/hmm-*` に89ディレクトリ(報告書付き21本、残りは設計・計画・子実行)。全てGit対象外。全研究で `candidateEligible=false`、評価期間は探索済みで未使用OOSではない。採用・発注に進んだものはない
- **第1段 直接予測 (09-12)**: M60/M15/M30 モメンタム+HMM高ボラ拒否(regime-v1 は完全24時間足の前提が崩れて中断→v2、timeframes)、4本モメンタム(short)、M1特徴量アブレーション(m1-features)。**2bpsで全セルPF<1**(regime-v2 はHMM側 0.64〜0.93)。次足予測の診断(predictive)は予測/実現相関 ±0.0085、ゼロ予測比MSE 0.9999〜1.0003 で実用的な誤差削減なし。Christensen型(paper-frequency)のD1は翌日物金利の欠落でブロック
- **FX日足 (hmm-d1-price-v1)**: 150セル中125完了、金の25セルは共通訓練行400未満でブロック。2bpsで銘柄×特徴量25プール中 **PF>1 は5、同一約定の always-long を上回ったのは3**(USDJPY shape、AUDNZD baseline・shape)。weekend_hold の過大計上(10,620→5,300)は `ERRATA.md` で訂正済み、PFには影響なし
- **先物 (09-13)**: 日足(futures-d1、150/150完了)は株価指数の上昇で **30プール中15がPF>1、ただし always-long 超えは3**。H1/H4/D1一致(futures-mtf)は18中2がPF>1(WTI 1.120、Brent 1.067)だが、上位5勝ち除外で WTI +2001→+93bps、Brent +906→−223、10bpsでは全18がPF<1。予測対象(i+1 の r1)と約定区間(i+2→i+3)のずれは REPORT.md に明記済み。SMA20追加(mtf-ma)、LPC/ケプストラム4・12次と差分、5/15/30分足(spectral 系5本)は一貫した改善なし。5/15分足は元データのスパン不足で主期間がブロックされ、2026-09の9営業日パイロットのみ
- **写像とゲート (09-14〜15)**: 状態→収益の写像を約定区間に揃える(execution-aligned)、符号付き r1/r5/r20 追加(signed-features)、訓練中央値による棄権(abstention: 主期間2bpsで54/54が機会あたり改善、ただし**35/54はコスト前で改善なし=取引削減の効果**)、スペクトル窓の名目時間合わせ(duration-matched、主期間は同一性対照のみ)、訓練6/12か月・係数4/8/12次(training-dimension: 改善/悪化が拮抗し採用なし)。これら後続研究の2bps PF>1 は各研究のセルの0〜19%(パイロットを含み、セル同士は同一市場・同一アームで重複する)
- **既存シグナルのゲート化 (09-15〜17)**: 基準を元の M15 VWAP+EMA9/20 に固定し(signal-baseline/stream)、HMMを「取引を通すか」のゲートに転換(signal-gate: 6銘柄×42か月=252スロット)。実行中に一時ライブラリの hmmlearn メタデータが消失して中断 → Python 3.12 の封印ランタイム(`hmm-runtime-py312-v1`、7,142ファイルをハッシュ固定)と、スロット単位の永続claim・再試行禁止の継続実行器(v1〜v3、isolated)を整備。250スロット記録、2スロット不明(GBPUSD 2023-09、USDJPY 2024-07、再試行なし)
- **支持不足の診断 (09-17〜18)**: ゲートの写像は方向ごとに各状態の質量20・100エピソードを要求したが、元特徴量では502写像中4しか満たさなかった。方向不変の特徴量(environment-support)で108/500に改善したが386が質量不足。切り分け: 状態全体では足りている(entry_conditioned_deficit 382)、UTC時刻を揃えても(379)、方向機会を揃えても(379)変わらず、**エントリーの99.5%は方向条件の発生直後、参照バーの90.8%は継続中**(onset)。エントリー時点の状態分布がシグナルの構造上偏っていることが原因
- **エントリー信頼度ヘッド (09-18)**: 市場状態(事後確率q)とエントリー信頼度を分離し、qへのリッジ回帰ヘッドを**合成データのみ**で検証(局所支持、履歴量200/400/800、古い履歴での関係反転、半減期50日の減衰、構造変化後の適応、成熟ラベルの逐次到着)。実データのエントリー支持の事前確認(real-support)と、XAUUSD の事後確率欠損 2,473/6,032件の出所監査(missing-q: 全件が連続性条件、うち1,940件は NY 16:45〜18:00 の日次休場)を完了
- **長期校正の準備 (09-18〜19)**: 日次休場を跨ぐ特徴量の連続性規則(scheduled-closure、合成のみ・14テスト凍結)と、9か月学習/12か月校正/1か月評価の長期校正(fx-long)の境界検証・実ライブラリでの合成実証・2021-04以降のM15入力準備(5通貨)
- **長期校正の実データ実行 (09-23)**: 実行器 `hmm-fx-long-historical-v1` を実装前の計画書から作成し、独立レビューを4回経て実行。1回目は BLOCK(`--worker` を直接呼ぶと承認も claim もなしで学習でき、失われたスロットを再実行できた。1スロット300秒の上限が負荷下でスロットを失わせる)。worker 側で承認・claim の内容・開始記録・親PID・`failure.json` 不在を確認してから一度きりの出力ディレクトリを学習前に作る形に改め、上限を1800秒に広げたうえで本番規模の canary と時間チェックを置き、どの停止も claim の前に起きるようにした。31テスト、変異29件+7件を全件検出。main と reviewer の承認を4ファイルのハッシュに束縛して1回だけ実行: **208/208 学習成功、失われたスロットなし、29分**。入力だけで決まる数(3か月strict200の完全窓360/416、無効q 2,653、窓不足3,985、評価インテント33,176)は過去の段階と完全一致
- **支持の結果(33,176インテント中、支持率80%以上の方向数/420)**: 3か月 strict200 は 21,453(64.66%、77)、3か月 strict800 は窓が埋まらず0。12か月 strict200 は 24,731(74.54%、93)、**12か月 strict800 は 27,195(81.97%、完全窓401/416、335)**、12か月 strict800減衰は 25,541(76.99%、207)。12か月と3か月の strict200 比較は機械的に決まる: 3か月の窓が200件を満たせば直近200件は同一なので支持も同一で、12か月だけの3,278件は全て3か月の窓が埋まらなかった56方向から来る。**履歴を延ばす実質的な効果は12か月でしか埋まらない800窓に表れる**。無効な事後確率(8.0%)は窓に関係なく上限を下げ、AUDNZD と USDJPY は支持率76〜77%と低い。支持は予測力・収益性ではなく、同一モデル内の比較で、期間は全て探索済み
- **実行後の独立検証 (09-23)**: 実装にもレビューにも関わっていない検証者が、集計・組み立て・ダイジェストを再実装し、凍結済みの参照実装だけを呼んで照合。**VERIFIED_WITH_NOTES、不一致0件**: 成果物424件のハッシュ、208件の claim・入力ダイジェスト・モデル、保存パラメータからの再フィルタ(再学習なし)による事後確率、組み立てた入力と支持の行、`summary.json` の全キーが一致
- **性能実験の設計 (09-23)**: 探索済み履歴に限定した探索実験として設計(前向き検証の期間は予約しない。元の M1 データは第三者の改訂済み履歴で、manifest 自身が前向き・OOS 証拠を否定しており、2026-07 以降は他の研究がすでに参照していた)。元シグナルはコスト後に全通貨で負けるため、ゲートなしに対する改善は取引を減らすだけで得られる(棄権研究と同じ罠)。そこで主要対照を同じ行で学習した定数ヘッドとし、日付単位でラベルをずらして学習し直すスキルなし対照(1,000回)、平均損益のゆっくりした流れを拾う時間だけの対照、0bps での再計算、取引数・集中度の条件を加えた7条件を全て必須として事前に固定した。独立設計レビューは v1 BLOCK(ランダム拒否の対照が日付共通ノイズ下で偶然18〜21%通過し、シードも負値で落ちる。ラベルを見た後に決められる選択肢が残る。少数取引で通過できる。予約の方法が成立しない)から v2.2 で通過
- **性能実験の実行 (09-23)**: 独立コードレビューは1回目 BLOCK(マスクをラベル読み込み前に確定していない。ゲート3のテストが欠落・無効)。ラベルを使わない事前確定を名前空間を開く前に行い、そのハッシュを claim に記録する形に直して2回目で通過。期間利用を記録してから、2段階(学習ラベルだけで208スロットの判断を確定 → 評価結果を結合)で1回だけ実行、16分
- **性能実験の結果: 選別条件は不合格(7条件中、条件5のみ通過)**。主要比較 S(2) は −0.069 bps/機会(S(0) −0.037)で、スキルなし対照1,000回のうち977回がこれ以上。ヘッドが選んだ766件は平均 −2.58 bps で、照合した取引全体の平均よりわずかに悪い。通貨別・年別にプラスはなく、時間だけの対照にも負けた。ゲートなしに対する +2.40 bps は、選んだ取引の寄与 −0.07 と負けているベースラインを避けた寄与 +2.47 に分解され、取引削減の効果にすぎない。独立検証は VERIFIED(不一致0件、ビット単位で一致)。記帳の誤りを意図的に入れても符号は変わらなかった。**契約に従いこの系統は打ち切り、同じ履歴で条件を変えた再測定はしない**
- **日次休場の統合 (09-24)**: missing-q で見つかった金の欠損(NY 16:45〜18:00 の日次休場で特徴量がリセットされ、再開後の最初の判断が23:30になる)を scheduled-closure 規則で埋めた場合に、旧来の42区切り・3か月校正で学習窓が埋まるかを、ラベルを開かず学習もせずに数えた。対象は XAUUSD のみ(FX は打ち切った系統の再測定になるため除外)。FXTF の過去カレンダーの証拠がないため、月〜木の NY 17:00〜18:00 を休場と**仮定**する層を別に設け(`known_at` なし、規則ファイルのハッシュに束縛、採用候補の根拠にしない)、中止条件「84方向中42方向以上で200件の窓が埋まらなければ打ち切り」を事前に固定した。計画レビューは1回目 BLOCK(数える区切りが未指定で中止条件が空振りしうる、凍結版は実データで動かない)、コードレビューは1回目 BLOCK(承認前に実データで照合だけを試す `--rehearse` が、不一致を見てから直して承認を得る抜け道になる → 削除)。承認後に1回だけ実行、6秒
- **日次休場の統合の結果: 中止条件で打ち切り**。旧規則は保存済みの事後確率の有無・方向ごとの件数・集計(0/84、61/126.5/152)を全件再現した。仮定した休場では **200件の窓が埋まった方向は25/84**(基準42)、800件は0、方向あたりの件数は 85/189/232。校正窓内の特徴量利用可能な延べ件数を5,396件回復し、失われた延べ件数は0。**校正の件数は重複する3か月窓をfoldごとに数えた延べ件数であり、異なるエントリー数や独立標本数ではない**。日次休場による欠損は校正の延べ5,657件から0へ減少し、評価でも0になったが、週末リセット(校正の延べ1,315件)と3か月の校正期間のため多くの方向が200件に届かない。休場時間帯に足がある43日は機械的に除外(2022-11〜2023-01と2023年夏に集中)。独立検証は VERIFIED(107/107、不一致0件)。境界 198〜202 のどれでも21〜28方向で、結論は変わらない。**カレンダーの証拠に依存しない結果なので、FXTF への問い合わせはこの系統には不要**
- **結論**: 日中足では、HMM状態を直接の売買方向にしても既存シグナルのゲートにしても、2bps後に一貫してPF 1を超える構成はなかった。日足では上昇相場でPF 1を超えるものがあったが、同じ約定での always-long を上回ったのは FX 3/25、先物 3/30 にとどまる。長期校正では、12か月の履歴で800件の窓が埋まり、エントリー信頼度ヘッドの局所支持が64.66%から81.97%に上がった(支持のみ、損益は未測定)。ただし、判断材料が揃うことと判断が当たることは別で、12か月・800件で学習したエントリー信頼度ヘッドは取引を選ぶスキルを示さなかった(性能実験、不合格)。金では日次休場を跨いで特徴量をつないでも、3か月校正で200件の窓が埋まったのは25/84方向にとどまった(件数のみ、損益は未測定)
- **残**: 性能実験は不合格で打ち切り(副次の strict200 系は 0〜2bps でわずかにプラスだったが、契約上、副次の結果で前向き検証に進むことはできない)。scheduled-closure 規則の統合も中止条件で打ち切り。hmm-d1-price-v1 の weekend 判定を時刻述語に置き換える v2 は PF に影響しないため保留
