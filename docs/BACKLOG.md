...
- **結論**: 日中足では、HMM状態を直接の売買方向にしても既存シグナルのゲートにしても、2bps後に一貫してPF 1を超える構成はなかった。日足では上昇相場でPF 1を超えるものがあったが、同じ約定での always-long を上回ったのは FX 3/25、先物 3/30 にとどまる。長期校正では、12か月の履歴で800件の窓が埋まり、エントリー信頼度ヘッドの局所支持が64.66%から81.97%に上がった(支持のみ、損益は未測定)。ただし、判断材料が揃うことと判断が当たることは別で、12か月・800件で学習したエントリー信頼度ヘッドは取引を選ぶスキルを示さなかった(性能実験、不合格)
- **残**: 性能実験は不合格で打ち切り(副次の strict200 系は 0〜2bps でわずかにプラスだったが、契約上、副次の結果で前向き検証に進むことはできない)。(1) scheduled-closure 規則の実モデル統合、(2) hmm-d1-price-v1 の weekend 判定を時刻述語に置き換える v2 は PF に影響しないため保留
