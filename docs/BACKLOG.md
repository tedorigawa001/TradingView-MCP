...
- **残タスク**: 認証済みCME日次統計provider (DataMine API / FTP Bulletin 自動取得)、preliminary/final版およびfirst-seenタイムスタンプのPoint-in-Time保存(集計OIも改訂vintageを持たないため、観測ラグで公表遅延は吸収できても改訂は補正できない)、限月・expiry・roll calendar判定、先物現物スプレッド(Basis)追跡、CME確報Volumeとの独立検証、2022-12以前へ遡れる集計OI系列の入手、外部ラベル系列を将来結果へ結合しジャーナル記録する経路(現在は手計算の証拠を記録する手段がない)、実機における複数銘柄（6E, 6J, 6B, 6A, 6C, 6S, 6N, ES, NQ, YM, GC, SI, CL）の継続実地検証。
