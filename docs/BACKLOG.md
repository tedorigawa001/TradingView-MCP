...
### #100 HMM 研究系列(研究実施, 2026-09-12〜09-19)
...
- **現在地 (09-18〜19)**: 日次休場を跨ぐ特徴量の連続性規則(scheduled-closure、合成のみ・14テスト凍結)と、9か月学習/12か月校正/1か月評価の長期校正(fx-long)の境界検証・実ライブラリでの合成実証・2021-04以降のM15入力準備(5通貨)まで完了。**実データでの長期校正の学習は未承認・未実行**、性能は未測定
- **結論**: 日中足では、HMM状態を直接の売買方向にしても既存シグナルのゲートにしても、2bps後に一貫してPF 1を超える構成はなかった。日足では上昇相場でPF 1を超えるものがあったが、同じ約定での always-long を上回ったのは FX 3/25、先物 3/30 にとどまる
- **残**: (1) fx-long の実データ学習を行うか(承認事項)、(2) scheduled-closure 規則の実モデル統合、(3) hmm-d1-price-v1 の weekend 判定を時刻述語に置き換える v2 は PF に影響しないため保留
