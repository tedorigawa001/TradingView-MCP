...
- **より大きな n でも結果は変わらなかった**: #68 で n=100,000 を推した根拠は「真の率が 4.73% なら通過確率 97.7%」だったが、実測の 4.924% に対しては **19.0%** にすぎない。300,000で48.0%、1,000,000で93.8%。**推奨自体は誤りではないが、それで決着したはずだという含みは誤りだった**
- **この失敗が意味すること**: 真の率が名目のすぐ下に張り付いているため、「Wilson上限 < 5%」という条件は**実行可能な n では満たせない**。これは条件を凍結した時点では見えず、測定して初めて分かった。**したがって本項の失敗は「偽陽性率が名目を超える」ことの証拠ではなく、「名目のすぐ下にあり、この条件では判定できない」ことの証拠である。**この但し書きなしに引用してはならない
