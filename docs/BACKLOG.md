...
- **唯一OOSで機構が再現した候補**: RSI2 × USDJPY × 1H × Tokyo のみ。3 fold すべてで Tokyo のみが選択され、3 fold すべてで test が黒字(1.276 / 1.107 / 2.036)。ゲートOFF の train expectancy は全fold負(−27.4 / −32.9 / −33.8)、Tokyo のみは全fold正(+19.3 / +24.3 / +20.4)で符号が一度も反転しない。ただし PF1.308 は目標1.4未達、64トレードのみであり、**採用ではなく「棄却されなかった唯一の候補」**である
