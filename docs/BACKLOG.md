...
- **主検定readiness表示(実装, 2026-07-29)**: `get_carry_core_primary_readiness`はチャートを触らず、core 6通貨のfirst-seen版から共通の初回利用可能日、60×20営業日の**最短**完了見込み日、欠損通貨を返す。これは全pairの確定日足が連続し、金利差ゼロのアンカー除外がない理想条件の下限であり、祝日・価格欠損・金利差ゼロを算入した保守的な完了日ではない。特に主検定は1ペアでも金利差がゼロならアンカー日全体を除外するため、過去のUSDCHF同値期のような状態は完了を後ろへ押す。政策金利ストアは値の変化だけを追記するため、同じ値だった日の収集成功をこの表示から証明できない。そこで `collection_continuity_status: not_proven_from_change_only_policy_rate_versions` を固定して返し、見込み日だけで主検定をreadyと誤認しない。実際の価格coverage・金利差ゼロ・主検定の評価可能性は引き続き `run_carry_core_primary_test` が判定する
