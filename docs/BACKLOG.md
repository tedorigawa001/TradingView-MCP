...
- **責務境界**: 読み取り専用で、注文・約定・スリッページ・損益を計算しない。評価ログへの追記も行わず、必要な場合は既存Evaluation Pipelineが返却結果を明示的に記録する
...
- **案**: `evaluation_timeframe` パラメータを追加し、指定時のみ「時間足を一時切替→OHLCV取得→元の時間足へ復元」を1ツール内で行う。`run_backtest` の一時適用→復元パターンを流用。復元失敗は隠さず結果に明示する(`restored: false` + 元の時間足)。切替はチャート状態に触れるため、`set_timeframe` と同じ操作系クラスとして扱う
- **注意点**: 切替後のバー未ロード検知(#4と同じ `bars: 0` 拒否)、評価中に他ツールがチャートを読む競合、`expected_timeframe`(オーバーレイの検証対象)と `evaluation_timeframe`(証拠の解像度)の区別をレスポンスに明記すること

### #20 分析ジャーナル(記録と較正、優先度: 高・規模: 中)
...
- **案**: (1) `apply_analysis_overlay` の confirm 確定時に分析全体(analysisId・全水準・confidence・note)をローカルJSONLへ自動追記、(2) `evaluate_analysis_overlay_outcome` の結果を analysisId で突合して明示的に追記する記録ツール(または evaluate 側の `record: true` オプション)、(3) 集計: シナリオ的中率、bias別・銘柄別成績、確信度の較正カーブ
- **設計方針**: 書き込み先はローカルファイルのみ(チャート・Pineライブラリに触れない)。既存 Evaluation Pipeline(`npm run evaluate` のJSONL)と形式を揃えて接続する。同一 analysisId の重複記録は追記自体は許し、集計側で最新評価を採用する(事後評価は履歴が伸びると `ongoing`→`complete` に変わり得るため)
