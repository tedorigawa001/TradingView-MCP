...
- グラフィックスの2形態はアプリ内部実装依存(`_` プレフィックス)。アプリ更新で壊れる可能性が最も高い箇所なので、抽出ロジックは `getIndicatorGraphics` 内に隔離し、統合テストで検知する
- `load_more_history` はロード済みデータをページに蓄積する(ユーザーが左スクロールした場合と同じ挙動)
- dwgtables(Smart Money の右上テーブル等)は未対応。必要になれば `dwgtablecells` から取得可能
