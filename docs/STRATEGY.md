...
- ✅ **バックログ #14 完了**(2026-07-09): `get_mtf_overview` の複数シンボル対応(`symbols: string[]`、上限20件)。実際の為替横断分析で判明したギャップに対応
- ✅ **バックログ #15 完了**(2026-07-09): `get_indicator_values` のノイズフィルタに `ohlc_*` type(`plotcandle()` のOHLCミラー)を追加。**全24ツール**(ツール数は変わらず既存ツールの拡張)
