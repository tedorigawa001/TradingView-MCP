...
export class FuturesOpenInterestFirstSeenStore {
   ...
   * symbol, scope and date. Re-reading an unchanged series is the normal case and must not grow the
   * log, while a preliminary value later corrected to a final one is exactly what has to be kept.
   */
  async observeMany(observations: FuturesOpenInterestObservation[]): Promise<{
    ...
    return this.log.serialize(async () => {
      ...
      const candidates = observations.map((observation) => {
        const key = `${observation.futures_symbol}|${observation.scope}|${observation.observation_date}`;
        if (seen.has(key)) {
      ...
      const latestFor = (candidate: FuturesOpenInterestRecord) => records
        .filter((record) => record.futures_symbol === candidate.futures_symbol &&
          record.scope === candidate.scope && record.observation_date === candidate.observation_date)
  ...
  async coverage(): Promise<{
    records: number;
    series: Array<{ futures_symbol: string; scope: FuturesOpenInterestScope; dates: number;
    ...
    return this.log.serialize(async () => {
      ...
      for (const record of records) {
        const key = `${record.futures_symbol}|${record.scope}`;
