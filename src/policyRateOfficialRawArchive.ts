...
import { syncDirectoryEntry, noFollowFlag, openExclusiveFile, posixModeEnforced } from "./fsDurability.js";
...
export class OfficialPolicyRateRawArchive {
  ...
  async store(rawSha256: string, raw: string | Buffer): Promise<{ stored: boolean; bytes: number }> {
    ...
    try {
      const handle = await openExclusiveFile(path, "official policy-rate raw archive");
      try {
