...
| `get_trade_decision_context` | 対象チャート、OHLC、キーレベル、MTF、イベント、COT、実質金利、bid/askを1つの`snapshot_id`へ統合。方向推奨やチャート操作は行わない |
...
- このツールは**読み取り中心**です。書き込みは自作Pineの非破壊保存、チャート上の自作Pine追加・入力変更・所有確認付き削除、監査済み分析からの期限付き価格アラート作成に限定しています。`save_pine_script`、`apply_analysis_overlay`、`ensure_analysis_overlay`、`remove_owned_study`、`create_analysis_alerts`はconfirmフローを持ちます。注文、既存アラートの変更・再開・削除、Webhook、ウォッチリスト変更、Pineライブラリのスクリプト削除は意図的に実装していません
