...
| `run_market_event_study` | アクティブチャートの確定OHLCで条件付きイベントスタディを実行。session auctionを受容/失敗へ排他分類し、複数horizonのreturn・MFE・MAE・target到達時間、fold、平均/Wilson信頼区間、申告試行数と未補正条件を返す |
