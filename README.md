...
| `evaluate_analysis_overlay_outcome` | 監査済み分析を、分析後のロード済み確定足で事後評価。Entry→任意Confirmation後のTarget 1/Stop初回到達を判定し、確認前のInvalidation、同一足・ギャップ・履歴不足を保守的に処理 |
...
| `ensure_analysis_overlay` | オーバーレイを冪等に準備。現行版は再利用、未配置なら追加、旧版なら14入力を移行・検証後に入れ替え。**confirm なしはドライラン** |
| `apply_analysis_overlay` | 専用オーバーレイへ構造化分析を反映。銘柄・時間足・入力契約をfail-closedで検証し、**confirm なしはドライラン**。反映後に入力値と描画数を読み戻す |

分析結果をチャートへ重ねる場合は、テンプレートを一度だけ`save_pine_script`で保存し、以後は`ensure_analysis_overlay`で配置・更新・`study_id`取得を行います。同じ現行版があれば書き込みなしで再利用されるため、分析ごとにPineスクリプトやスタディを増やす必要はありません。期限切れの分析は拒否せず、チャート上で`EXPIRED`表示になります。反映前の初期値は実分析として扱わず、`get_analysis_overlay_status`が`unconfigured`を返します。反映後は同ツールで入力・現在価格・描画の整合性を再確認できます。現在値判定は過去の到達順序を示しません。到達順序は`evaluate_analysis_overlay_outcome`が、分析時刻を含む足と形成中足を除外した確定OHLCで評価します。Entry後、Confirmation設定時は後続足での確認を待ち、それ以前のInvalidation接触はシナリオ取消として扱います。有効化後はTarget 1対Stopのfirst-hitを評価します。これは約定や損益を証明するものではなく、同一足で相反水準へ触れた場合や始値ギャップでは`ambiguous`を返します。履歴が不足する場合は`load_more_history`後に再評価します。`D`/`W`は対応し、暦月`M`は期間が可変なため`not_evaluable`です。
...
- このツールは**読み取り中心**です。書き込みは自作Pineの非破壊保存、チャート上の自作Pine追加・入力変更・所有確認付き削除に限定しています。`save_pine_script`、`apply_analysis_overlay`、`ensure_analysis_overlay`、`remove_owned_study`はconfirmフローを持ちます。注文・アラート作成・ウォッチリスト変更・Pineライブラリのスクリプト削除は意図的に実装していません
