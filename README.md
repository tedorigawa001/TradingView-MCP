...
For a repository-location-independent launchd setup, install the package globally (`npm install -g bushido-tradingview-mcp`) and customize the absolute Node and global-module paths in the examples. The [research health agent](docs/launchd/com.tradingview-mcp.collection-health-research.plist.example) runs hourly on all seven days, so a Friday-night outage is detected within the two-hour threshold. The [first-seen health agent](docs/launchd/com.tradingview-mcp.collection-health-first-seen.plist.example) runs 45 minutes after each collection window. The separate [policy-rate health agent](docs/launchd/com.tradingview-mcp.collection-health-policy-rate.plist.example) runs at 11:15 JST on weekdays, after the 10:45 collector. These agents and the first-seen collector do not use `WorkingDirectory`; moving the source repository does not break the installed jobs.
...
| `summarize_backtest_ledger` | Recomputes trade-level PF and net bps with slice denominators and missing-outcome counts, without chart access. Optional `research_id` records local slice exploration counts before returning metrics; otherwise explicitly untracked. See [ledger import contract](docs/BACKTEST_LEDGERS.md) |
