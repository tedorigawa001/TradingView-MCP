...
分析結果をチャートへ重ねる場合は、テンプレートを一度だけ`save_pine_script`で保存し、`add_pine_to_chart`で追加します。以後は同じ`study_id`に対して`apply_analysis_overlay`を呼ぶため、分析ごとにPineスクリプトやスタディを増やす必要はありません。期限切れの分析は拒否せず、チャート上で`EXPIRED`表示になります。
...
- このツールは**読み取り中心**です。書き込みは自作Pineの非破壊保存、チャートへの自作Pine追加、既存スタディ入力の変更に限定しています。`save_pine_script`と`apply_analysis_overlay`はconfirm必須です。注文・アラート作成・ウォッチリスト変更・スクリプト削除は意図的に実装していません
