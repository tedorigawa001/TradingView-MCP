...
分析結果をチャートへ重ねる場合は、テンプレートを一度だけ`save_pine_script`で保存し、以後は`ensure_analysis_overlay`で配置・更新・`study_id`取得を行います。同じ現行版があれば書き込みなしで再利用されるため、分析ごとにPineスクリプトやスタディを増やす必要はありません。期限切れの分析は拒否せず、チャート上で`EXPIRED`表示になります。反映前の初期値は実分析として扱わず、`get_analysis_overlay_status`が`unconfigured`を返します。反映後は同ツールで入力・現在価格・描画の整合性を再確認できます。価格判定は取得時点の形成中バー終値との比較であり、過去に各水準へ到達した事実や到達順序を示すものではありません。
